...
while IFS=\n read -r line
	...
	games_played=$(echo "$line" | awk -F'[, ]+' '{print $9}')
	...
	rows+=("$username,$date,$time,$games_played,$user_wins,$greg_wins,$winner_attempts")
done < "$logs"
...
for row in "${rows[@]}"; do
	IFS=',' read -r username date time games_played user_wins greg_wins winner_attempts<<< "$row"
	printf "%-10s %-15s %-10s %-10s %-10s %-10s %-10s\n" "$username" "$date" "$time" "$games_played" "$user_wins" "$greg_wins" "$winner_attempts"
done
